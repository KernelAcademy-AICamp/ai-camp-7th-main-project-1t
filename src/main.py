...
- matchProfile: 전문가(팀원) 매칭 점수 계산에 쓰는 구조화 속성. 아래 '허용 값'에서만 고르세요.
    ...
    · domains    : 프로젝트 분야. [복지, 교육, 반려동물, 지역 커뮤니티, 의료, ESG, 콘텐츠, 생산성, 음식·외식, 커머스·유통, 라이프스타일, 금융, 여행, 물류·모빌리티, 환경] 중 1~2개. 반드시 아이디어와 직접 관련된 분야만 고르세요. 딱 맞는 게 없으면 억지로 2개를 채우지 말고, 가장 가까운 1개만 고르세요.
...
합류 지원자의 이력서(프로필)와 실제 제출한 답변을, 이 프로젝트의 기획안·매칭 요소·필수 질문과 비교해
프로젝트와 얼마나 잘 맞는지 매칭률(0~100%)을 매깁니다. 지원자가 하지 않은 말을 지어내지 마세요.

규칙:
- 반드시 아래 JSON 형식으로만 답하세요. JSON 외 다른 말이나 코드펜스(```)를 절대 붙이지 마세요.
- answers: 주어진 필수 질문 각각에 대해, 지원자가 실제로 제출한 답변(answer)을 그대로 옮기고(창작·수정 금지), 그 답이 적합한지 한 줄 코멘트(comment). 지원자가 답하지 않은 질문은 answer를 빈 문자열("")로 두고 절대 추측하지 마세요.
...
{
  "answers": [
    {"question": "질문", "answer": "지원자가 제출한 실제 답변(없으면 빈 문자열)", "comment": "적합성 코멘트"}
  ],
...
- 선택지는 반드시 '입력한 아이디어의 핵심 고객/강점'에 한정하세요. 아이디어와 직접 관련 없는
  부수적·파생적 대상(예: 선물 구매자, 대리 구매자 등)은 넣지 말고, 그 서비스를 실제로 쓰는
  1차 사용자를 기준으로 제시합니다.
- 한국어로 짧고 명확하게.
